...
function renderCatalog() {
  main.innerHTML = `<section class="hero"><p class="eyebrow">SECURE. SEARCHABLE. CURRENT.</p><h1>장비의 최신 펌웨어를<br><span>한곳에서 확인하세요.</span></h1><p>모델명과 설치 버전을 입력해 업데이트 여부를 바로 확인하세요.</p><form id="quick-check" class="quick-check"><label><span>모델명</span><input name="model" required placeholder="DR-6316PS"></label><label><span>현재 버전</span><input name="version" required placeholder="3.1.0"></label><button class="primary" type="submit">버전 비교</button></form><div id="quick-result" class="quick-result" aria-live="polite"></div></section>
